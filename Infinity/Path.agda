...
module _ {A : Set ℓ₁} where
  ...
  cong : ∀ {B : A → Set ℓ₂} {x y : A} (f : (a : A) → B a) (p : x ≡ y) → PathP (λ i → B (p i)) (f x) (f y)
  cong f p = λ i → f (p i)
...
comp↑ {A = A} a₁ a₂ b₁ b₂ p q r = λ i → primComp (λ _ → A) _ (λ j → λ { (i = i0) → p j ; (i = i1) → q j }) (r i)

comp↓ : ∀ {A : Set ℓ} (a₁ a₂ b₁ b₂ : A) (p : a₁ ≡ a₂) (q : b₁ ≡ b₂) (r : a₂ ≡ b₂) → a₁ ≡ b₁
comp↓ a₁ a₂ b₁ b₂ p q = comp↑ a₂ a₁ b₂ b₁ (sym p) (sym q) 
