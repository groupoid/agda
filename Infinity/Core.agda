...
-- Introduced with lambda abstraction and eliminated with application, just like function types.
...
-- * There are cubical subtypes as in CCHM. Note that these are not fibrant (hence in Setω):
...
comp A {φ = φ} u u0 = hcomp (λ i → λ { (φ = i1) → transp (λ j → A (i ∨ j)) i (u _ 1=1) })
                            (transp A i0 (ouc u0))

-- Heterogeneous filling defined using comp
fill : ∀ {ℓ : I → Level} (A : ∀ i → Set (ℓ i)) {φ : I} (u : ∀ i → Partial φ (A i))
         (u0 : A i0 [ φ ↦ u i0 ]) → ∀ i →  A i
